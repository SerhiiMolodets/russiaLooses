...
class LoosesListViewModel: LoosesListViewModelProtocol {
    ...
    func reduceTotalLoses() -> [String: Int] {
        ...
        if let lastDayEquipment = dataService.equipmentLooses.max(by: { $0.day ?? 0 < $1.day ?? 0 }), let lastDayPersonnel = dataService.personnelLooses.first(where: { $0.day == lastDayEquipment.day }) {
